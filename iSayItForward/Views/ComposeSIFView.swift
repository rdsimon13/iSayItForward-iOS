...
struct ComposeSIFView: View {
    ...
    // MARK: - Mock Templates
    @State private var mockTemplates: [Template] = [
        Template(name: "Sunset Bliss", imageName: "sunset_placeholder"),
        Template(name: "Ocean Whisper", imageName: "ocean_placeholder"),
        Template(name: "Minimal Calm", imageName: "calm_placeholder")
    ]
    
    // MARK: - Init (for Edit/Resend compatibility)
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                // MARK: - Navigation Link (Confirmation)
                NavigationLink(
                ...
                // MARK: - Global Bottom Navigation Bar
                VStack {
                    Spacer()
                    BottomNavBar(
        ...
        .sheet(isPresented: $showTemplateGallery) {
            TemplateGalleryView(selectedTemplate: $selectedTemplate)
                .environmentObject(router)
                .environmentObject(authState)  // ✅ Replaces AppState()
        }
        .sheet(isPresented: $showSignatureView) {
            SignatureView(isPresented: $showSignatureView) { signatureData in
                if let image = UIImage(data: signatureData.signatureImageData) {
                    savedSignature = image
                }
            }
            .environmentObject(router)
            .environmentObject(authState)  // ✅ Add missing authState here
        }
    ...
    private var templateSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 14) {
                    ForEach(mockTemplates, id: \.self) { template in
                        VStack {
                            Image(template.imageName)
                                ...
                                .overlay(
                                    RoundedRectangle(cornerRadius: 12)
                                        .stroke(selectedTemplate?.id == template.id ? Color.blue : Color.clear, lineWidth: 3)
                                )
                                .onTapGesture {
    ...
    private func sendSIF() {
        ...
        Task {
            isSending = true
            print("🚀 sendSIF() function executing for user: \(userId)")
            
            do {
                let recipients = selectedFriends.map {
                    SIFRecipient(name: $0.name, email: $0.email)
                }
                ...
                print("🧾 Prepared SIF object — attempting Firestore write...")
                try await sifService.sendSIF(sif, for: userId)
                print("✅ Successfully wrote SIF to Firestore with ID: \(sif.id ?? "unknown")")
                ...
                alertMessage = "⚠️ Failed to send SIF: \(error.localizedDescription)"
                showingAlert = true
            }
            
            isSending = false
            print("🏁 sendSIF() task complete.")
        }
    }
    
    // MARK: - Placeholder SIF (for navigation fallback)
    private var placeholderSIF: SIF {
        SIF(
            senderId: "demoUser",
            recipients: [
                SIFRecipient(name: "Placeholder Recipient", email: "placeholder@example.com")
            ],
            subject: "Demo Subject",
            message: "This is a placeholder SIF used for preview or navigation fallback.",
            category: "General",
            tone: "Supportive",
            emotion: "Joyful",
            templateId: nil,
