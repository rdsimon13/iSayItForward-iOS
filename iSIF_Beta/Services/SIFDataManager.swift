...
// MARK: - SIF Data Manager
@MainActor
final class SIFDataManager: ObservableObject {
    ...
    func sendSIF(_ sif: SIF, for userId: String) async throws {
        ...
        guard let id = newSIF.id else {
            throw NSError(domain: "SIFDataManager", code: 0,
    ...
    func sendSIF(_ sif: SIF) async throws {
        ...
        var newSIF = sif
        if newSIF.id == nil { newSIF.id = UUID().uuidString }
        guard let id = newSIF.id else { throw NSError(domain: "SIFDataManager", code: 0) }
    ...
    func scheduleSIF(_ sif: SIF, for userId: String) async throws {
        ...
        guard let id = scheduledSIF.id else {
            throw NSError(domain: "SIFDataManager", code: 2,
    ...
    func updateSIF(_ sif: SIF, for userId: String) async throws {
        guard let id = sif.id else { throw NSError(domain: "SIFDataManager", code: 1) }
                ...
                if let error = error {
                    ...
                    ref.downloadURL { url, error in
                        if let url = url {
                            ...
                            continuation.resume(throwing: error ?? NSError(domain: "SIFDataManager", code: -1))
                        }
                    }
                }
            }
